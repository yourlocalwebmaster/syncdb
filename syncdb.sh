...
backupFile=${saveBackupPath}/${db_A_name}_backup_${current_time}.sql
tmpFile=${saveBackupPath}/${db_B_name}_tmp.sql

if [ -w ${saveBackupPath} ]; then
    ## Backup Current Database
    if mysqldump -h ${db_A_host} -u ${db_A_user} -p${db_A_pass} ${db_A_name} > ${backupFile}; then
    ## Export Database B
        if mysqldump -h ${db_B_host} -u ${db_B_user} -p${db_B_pass} ${db_B_name} > ${tmpFile}; then
    ## Clear current database.
            if mysql -h ${db_A_host} -u ${db_A_user} -p${db_A_pass} ${db_A_name} < ${dir}/droptables.sql; then
    ## Import Database B into Database A

                if mysql -h ${db_A_host} -u ${db_A_user} -p${db_A_pass} ${db_A_name} < ${tmpFile}; then
                     echo "\r\n";
